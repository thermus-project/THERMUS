...
Int_t lhc5020_fit_charm(Bool_t rWrite = 1, Bool_t gsfixed = 1, Bool_t fitMuQandS = 0, Bool_t constrainMuQ = 1,
                        Bool_t volCor = 0, Bool_t quantRes = 1)
{
   ...
   if (radius && gDebugMode)
      printf("INFO: radius = %.1f fm, volume = %.2f fm^3\n", radius, volume);
   // Double_t B=197, Q=79, Bover2Q = B/(2*Q); if(debugMode) printf("INFO: expected colliding system: Au-Au\n");
   Double_t B = 208, Q = 82, Bover2Q = B / (2 * Q);
   if (gDebugMode)
      printf("INFO: expected colliding system: Pb-Pb\n");
   ...
   if (gsfixed)
      ...
      par.FitGammas(gammas, 0.75, 1.25, 0.0001);
   //
   //--- gamma c&b:
   ...
   if (gDebugMode)
      printf("INFO: now generate Yields\n");
   fit.GenerateYields();
   ...
   if (gDebugMode)
      printf("INFO: this is the chi2 %.3f and quadratic deviation %.3f \n", fit.GetChiSquare(), fit.GetQuadDev());
   ...
   TTMYield *lYield = 0;
   Int_t idYields[] = {211, 321,        310,        2212,        3122,       3312, 3334, 313,
                       333, 1000010020, 1010010030, -1010010030, 1000020030, 411,  421,  413};
   ...
   if (gsfixed)
      printf("INFO: gamma_s was fixed to unity \n");
   else
      printf("INFO: gamma_s was a free parameter \n");
   if (volCor)
      printf("INFO: volume corrections were included \n");
   else
      printf("INFO: volume corrections were not included \n");
   if (quantRes)
      printf("INFO: quantum Stat and Resonance Width were included \n");
   else
      printf("INFO: quantum Stat and Resonance Width were not included \n");
   ...
   if (radius)
      printf("INFO: radius = %.2f +- %.2f fm, volume = %.0f +%.0f -%.0f fm^3 \n", radius, radiusError, volume,
             volume_hi - volume, volume - volume_lo);

   if (rWrite) {
      Bool_t saveOk = mySaveFitParameters(fileOutName, fit);

      if (saveOk)
         printf("***** done with saving fit information \n\n");
   }
...
Int_t mySaveFitParameters(TString &fileOutName, TTMThermalFitBSQ &fitresult)
{
   ...
   TTMYield *lYield = 0;
   Int_t idYields[16] = {211, 321,        310,        2212,        3122,       3312, 3334, 313,
                         333, 1000010020, 1010010030, -1010010030, 1000020030, 411,  421,  413};
   Int_t nCharges = 16;
   ...
   fileOut << "T\t= " << fitresult.GetParameterSet()->GetTPar()->GetValue() << " +- "
           << fitresult.GetParameterSet()->GetTPar()->GetError() << endl;
   fileOut << "gammas\t= " << fitresult.GetParameterSet()->GetGammasPar()->GetValue() << " +- "
           << fitresult.GetParameterSet()->GetGammasPar()->GetError() << endl;
   ...
   fileOut << "volume\t = " << Form("%.0f", volume) << " + " << Form("%.0f", volume_hi - volume) << " - "
           << Form("%.0f", volume - volume_lo) << endl;
   ...
   for (Int_t iYields = 0; iYields < nCharges; iYields++) {
      ...
      fileOut << Form(" %-15d", lYield->GetID1()) << Form("%-20s", lYield->GetTMName().Data()) << "   "
              << Form("%7f", lYield->GetModelValue()) << " \t" << Form("%7f", lYield->GetStdDev()) << endl;
      /*

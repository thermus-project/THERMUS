...
Int_t BSQConstrainSQCb(TTMThermalModelBSQ *model)
{
    ...
    if (model->GetParameterSet()->GetB2Q() == 0.){
        ...
        gsl_vector_set(x, 3, model->GetParameterSet()->GetParameter(8)->GetValue());
        Int_t check = 0;
        PARAMETERSS p;
        p.p0 = model;
        p.p1 = 0.0;
        ...
        if (check) {
            ...
            model->GetParameterSet()->GetParameter(8)->SetStatus("(Unable to constrain)");
            ...
            model->GetParameterSet()->GetParameter(8)->SetValue(0.);
            rv = check;
        } else {
            ...
            model->GetParameterSet()->GetParameter(8)->SetStatus("(*CONSTRAINED*)");
        }
        gsl_vector_free(x);
    }
    return rv;

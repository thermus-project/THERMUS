...
Int_t BSQfuncSQEN(const gsl_vector* x, void* p, gsl_vector* f)
{
    ...
    if (!check) {
        ...
        gsl_vector_set(f, 1, (model->GetBaryon() / 2. / model->GetCharge() - ene2) / ene2);
        
        Double_t ene3 = ((PARAMETERSS *)p)->p3;
        gsl_vector_set(f, 2, (model->GetEnergy() / model->GetDensity() - ene3) / ene3);
    } else {
